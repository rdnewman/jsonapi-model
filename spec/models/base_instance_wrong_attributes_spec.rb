RSpec.describe JSONAPI::Model::Base, type: :model do
  describe 'instance with validations and with wrong attributes', :vcr do
  ...
  describe 'instance without validations and with wrong attributes', :vcr do
    subject(:object) { klass.new(attributes) }
  ...
  describe 'instance with validations but with wrong attributes', :vcr do
    subject(:object) { klass.new(attributes) }
    ...
    describe '#id=' do
      before do
        allow(klass.connection)
          .to receive(:post)
          .and_return(MockClientError.bad_key)
      end
